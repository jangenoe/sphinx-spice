"""
sphinx_spice_file.nodes
~~~~~~~~~~~~~~~~~~~~~

Sphinx spice_file Nodes
...
class spice_file_node(docutil_nodes.Admonition, docutil_nodes.Element):
    gated = False


class spice_file_enumerable_node(docutil_nodes.Admonition, docutil_nodes.Element):
    gated = False
    resolved_title = False


class spice_file_end_node(docutil_nodes.Admonition, docutil_nodes.Element):
    pass
...
class spice_file_title(docutil_nodes.title):
    def default_title(self):
        title_text = self.children[0].astext()
        if title_text == "spice_file" or title_text == "spice_file %s":
            return True
        else:
            return False


class spice_file_subtitle(docutil_nodes.subtitle):
    pass
...
class spice_file_latex_number_reference(sphinx_nodes.number_reference):
    pass
...
def is_spice_file_node(node):
    return isinstance(node, spice_file_node) or isinstance(node, spice_file_enumerable_node)


def is_spice_file_enumerable_node(node):
...
def visit_spice_file_node(self, node: Node) -> None:
    if isinstance(self, LaTeXTranslator):
        label = (
            "\\phantomsection \\label{" + f"spice_file:{node.attributes['label']}" + "}"
...
def depart_spice_file_node(self, node: Node) -> None:
    if isinstance(self, LaTeXTranslator):
...
def visit_spice_file_enumerable_node(self, node: Node) -> None:
    """
    LaTeX Reference Structure is spice_file:{label} and resolved by
    spice_file_latex_number_reference nodes (see below)
    """
    if isinstance(self, LaTeXTranslator):
        label = (
            "\\phantomsection \\label{" + f"spice_file:{node.attributes['label']}" + "}\n"
...
def depart_spice_file_enumerable_node(self, node: Node) -> None:
    if isinstance(self, LaTeXTranslator):
        self.body.append(LaTeX.depart_admonition())
    else:
        self.body.append("</div>")
...
def visit_solution_node(self, node: Node) -> None:
    ...
    if isinstance(self, LaTeXTranslator):
        ...
        target_node = self.builder.env.sphinx_spice_file_registry[target_label]
...
def visit_spice_file_latex_number_reference(self, node):
    id = node.get("refid")
    text = node.astext()
    hyperref = r"\hyperref[spice_file:%s]{%s}" % (id, text)
    self.body.append(hyperref)
    raise docutil_nodes.SkipNode


def depart_spice_file_latex_number_reference(self, node):
    pass
